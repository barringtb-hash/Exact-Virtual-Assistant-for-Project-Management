...
export default async function handler(req, res) {
  ...
  const { docType, document } = resolveDocumentFromPayload(payload);
  if (!docType) {
  ...
  try {
    const buffer = await handler.render(document);
      ...
      render: async (document) => {
        try {
          ...
          if (isDocRenderValidationError(error)) {
            const payload = formatDocRenderError(error);
            console.error(
              `${docType} download docx render validation failed`,
              error
            );
