...
const manifestVersionCache = new Map();
...
function readManifestVersion(manifestPath) {
  ...
  if (manifestVersionCache.has(manifestPath)) {
    return manifestVersionCache.get(manifestPath);
  }
  ...
  try {
    ...
    const version =
      typeof parsed?.version === "string" && parsed.version.trim()
        ? parsed.version.trim()
        : null;
    manifestVersionCache.set(manifestPath, version);
    return version;
  } catch (error) {
    manifestVersionCache.set(manifestPath, null);
    return null;
  }
}
...
charterDocType.templateVersion = readManifestVersion(charterDocType.manifestPath);
...
ddpDocType.templateVersion = readManifestVersion(ddpDocType.manifestPath);
