...
describe('Assistant chat flows', () => {
  ...
  it('streams typed messages, syncs preview, and allows resending', () => {
    ...
    cy.get(composer).should('be.visible').type('Draft the kickoff agenda for next Monday{enter}');
    ...
    cy.get(composer).should('be.visible').type('Please resend the latest summary{enter}');
    cy.wait('@chatRequest');
  ...
  it('cancels an in-flight request when a new message is submitted', () => {
    ...
    cy.get(composer).should('be.visible').type('First attempt that will be cancelled{enter}');
    cy.wait(50);
    cy.get(composer).should('be.visible').type('retry this request with new context{enter}');
  ...
  it('recovers from network errors by allowing resend', () => {
    ...
    cy.get(composer).should('be.visible').type('trigger retry handling{enter}');
    cy.wait('@chatRequest');
