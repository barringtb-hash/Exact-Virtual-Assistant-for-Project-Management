...
test("/api/charter/finalize copies template, formats replacements, and returns document links", async () => {
  ...
  assert.equal(res.body.pdf, null);
...
test("/api/charter/finalize returns PDF when required fields are present", async () => {
  const schema = await loadCharterSchema();
  const state = createConversationState(schema);
  ...
  setField(state, "problem", { value: "Legacy CRM causes data silos." });
  ...
  global.fetch = async (url, options = {}) => {
    const urlString = typeof url === "string" ? url : url.toString();
    if (urlString.includes("copy_document")) {
      return createJsonResponse({ documentId: "drive-doc-456" });
    }
    if (urlString.includes("share_document")) {
      return createJsonResponse({ webViewLink: "https://docs.google.com/document/d/drive-doc-456/edit" });
    }
    if (urlString.includes("fetch")) {
      return createJsonResponse({ webViewLink: "https://docs.google.com/document/d/drive-doc-456" });
    }
    throw new Error(`Unexpected fetch call: ${urlString}`);
  };
  ...
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.document.id, "drive-doc-456");
  assert.equal(res.body.document.url, "https://docs.google.com/document/d/drive-doc-456");
  assert.ok(res.body.pdf);
  assert.ok(res.body.pdf.base64);

  const checklist = res.body.checklist || [];
  const missingRequired = checklist.filter((item) => item.missingRequired);
