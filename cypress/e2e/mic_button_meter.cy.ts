 ...
 * Tests the vertical audio meter fill inside the microphone button
 */

describe("Microphone Button with Integrated Meter", () => {
  beforeEach(() => {
    cy.visit("/");
  });
  ...
  it("shows a vertical fill inside the mic button when audio is present", () => {
    cy.window().then(async (win) => {
      ...
      cy.get('.mic-button .mic-button__fill').should("exist");

      // The meter should have a transform with scaleY > 0 (matrix with d value)
      cy.get('.mic-button .mic-button__fill').then(($el) => {
  ...
  it("meter is empty when mic is off", () => {
    ...
    cy.get('.mic-button .mic-button__fill').should(($el) => {
  ...
  it("works across different button sizes", () => {
    // This test verifies the meter fill is inside the button
    cy.window().then((win) => {
      cy.get(".mic-button").should("exist").then(($btn) => {
        const btnSize = $btn.width() || 0;

        // Verify meter fill exists inside the button
        cy.get('.mic-button .mic-button__fill').should("exist").then(($fill) => {
          const fillWidth = $fill.width() || 0;
          // Fill should span the full width of the button
          expect(fillWidth).to.be.at.least(btnSize - 10); // Allow for borders/padding
        });
      });
    });
  });
  ...
  it("respects reduced motion preferences", () => {
    cy.window().then((win) => {
      ...
      cy.get('.mic-button .mic-button__fill').should(($fill) => {
