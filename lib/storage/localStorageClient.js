...
const ID_INDEX_LOCK_FILENAME = ".local-storage-id-index.lock";
...
export class LocalStorageClient {
  ...
  getIdIndexLockPath() {
    return path.resolve(this.baseDirectory, ID_INDEX_LOCK_FILENAME);
  }

  async acquireIdIndexLock({ retries = 5, initialDelayMs = 20 } = {}) {
    const lockPath = this.getIdIndexLockPath();
    ...
    for (let attempt = 0; ; attempt += 1) {
      try {
        ...
        if (attempt >= retries) {
          throw new Error("Failed to acquire ID index lock");
        }
  ...
  async registerFileId(id, relativePath) {
    ...
    const lockHandle = await this.acquireIdIndexLock();
    try {
      ...
      try {
        ...
        try {
          await fs.unlink(this.getIdIndexLockPath());
        } catch (error) {
          if (error?.code !== "ENOENT") {
            throw error;
          }
